...
Nine cases. Each has exactly one grader on **how it got there** — a `tool_used: Skill` check — plus one or more graders on the **result**, so a run tells you both whether the output was right and whether SimplifyKit is what produced it.
...
| `core` | the other seven: `apply-implements-tasks`, `archive-merges-spec`, `archive-rejects-scenario-loss`, `propose-missing-ac-asks`, `ignores-unrelated-request`, `propose-does-not-auto-apply`, `propose-related-item-fills-gap` | 2–4 each | 0.8 |
